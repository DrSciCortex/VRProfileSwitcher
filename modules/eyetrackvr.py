...
Confirmed config location (from user):
  C:/Program Files (x86)/EyeTrackVR/eyetrack_settings.json

Also saves the VRCFT ETVR module config if present:
...
# Default install path (confirmed by user)
DEFAULT_ETVR_DIR = Path("C:/Program Files (x86)/EyeTrackVR")
CONFIG_FILENAME = "eyetrack_settings.json"
...
def _find_etvr_dir() -> Path | None:
    candidates = [
        ...
        Path(os.environ.get("LOCALAPPDATA", "")) / "Programs" / "EyeTrackVR",
        Path(os.environ.get("APPDATA", "")) / "EyeTrackVR",
    ]
    for c in candidates:
...
def _vrcft_module_config() -> Path:
    appdata = os.environ.get("APPDATA", str(Path.home() / "AppData" / "Roaming"))
    return Path(appdata) / "VRCFaceTracking" / "CustomLibs" / "ETVRModuleConfig.json"
...
class EyeTrackVRModule(VRModule):
    ...
    icon = "👁️"
    description = "EyeTrackVR eye tracking -- saves eyetrack_settings.json and optional VRCFT module config"

    def _etvr_dir(self) -> Path:
        override = self.options.get("etvr_dir")
        if override:
            return Path(override)
        return _find_etvr_dir() or DEFAULT_ETVR_DIR

    def _include_vrcft(self) -> bool:
        return bool(self.options.get("include_vrcft_module", True))

    def get_config_paths(self) -> list[Path]:
        paths = [self._etvr_dir() / CONFIG_FILENAME]
        if self._include_vrcft():
            paths.append(_vrcft_module_config())
        return paths
    ...
    def get_status(self) -> ModuleStatus:
        ...
        try:
            ...
            pass
        config = self._etvr_dir() / CONFIG_FILENAME
        return ModuleStatus(
    ...
    def backup(self, dest_dir: Path) -> tuple[bool, str]:
        ...
        # Main settings file
        src = self._etvr_dir() / CONFIG_FILENAME
        if src.exists():
            try:
        ...
        if self._include_vrcft():
            vrcft_src = _vrcft_module_config()
            if vrcft_src.exists():
                try:
                    shutil.copy2(vrcft_src, module_dest / "ETVRModuleConfig.json")
                    saved.append("ETVRModuleConfig.json")
                except Exception as e:
                    errors.append(f"ETVRModuleConfig.json: {e}")

        # Write manifest so restore knows the original location
        manifest = {
            "etvr_dir": str(self._etvr_dir()),
